...
public protocol HomeScenePayloadable {
  // var name: String { get }
  var delegate: ((Int) -> Void)? { get set }
}

@MainActor
public protocol HomeSceneBuildable {
  ...
  func build(with payload: HomeScenePayloadable) -> HomeSceneViewControllable
}
