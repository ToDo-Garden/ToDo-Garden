...
public struct SearchGardenSceneBuilder {
	...
	public struct Dependency {
		let someWorker: SomeWorkable
		let nextSceneBuilder: NextSceneBuildable
		
		public init(someWorker: SomeWorkable, nextSceneBuilder: NextSceneBuildable) {
...
extension SearchGardenSceneBuilder {
	...
	private func configureVIPCycle(for viewController: SearchGardenViewController) -> SearchGardenViewController {
		let interactor = SearchGardenInteractor(someWorker: self.dependency.someWorker)
		let presenter = SearchGardenPresenter()
		let router = SearchGardenRouter(nextSceneBuilder: self.dependency.nextSceneBuilder)
